...
- **arquivo/modalidade de destino** — ex.: "arquivo: tc_e.yml", "vai pro us.yml", "é de tomografia", "coloca no rx"
...
Opções da pergunta — apresente as que fizerem sentido, com a mais provável primeiro (baseado em palavras-chave do `replace`, ex. "tomografia"/"TC" → `tc_e.yml` primeiro; "ultrassom"/"USG" → `us.yml`; "radiografia"/"RX" → `rx_h.yml`; "ressonância"/"RM"/sinal em T1/T2 → `rm_a.yml`; "mamografia" → `mmg.yml`; texto genérico sem modalidade clara → `geral.yml`):

- `tc_e.yml` — Tomografia computadorizada
- `us.yml` — Ultrassonografia
- `rx_h.yml` — Radiografia
- `rm_a.yml` — Ressonância magnética
- `mmg.yml` — Mamografia
...
- **`rx_h.yml` / `tc_e.yml` / `rm_a.yml`**: dentro do banner de seção temático apropriado (`# === NOME DA SEÇÃO ===`). A taxonomia de 10 categorias está em `CONVENTIONS.md` (Crânio/Encéfalo/SNC, Face/Órbitas/..., Coluna, Tórax, Abdome/Pelve, Membros Superiores, Membros Inferiores, Vascular, Templates completos/Cabeçalhos/Assinaturas, Gerais/Diversos). Insira no fim do bloco da seção correspondente.
