...
- Full-report templates always use the same three section headings, in Title Case: `Técnica:` → `Análise:` → `Conclusão:`, each preceded by exactly one blank line and followed immediately by its content. The older spellings (`Descrição:`, `Opinião:`, `Impressão:`, `RELATÓRIO:`, and the ALL-CAPS variants) were normalized away — don't reintroduce them. See `CONVENTIONS.md`.
