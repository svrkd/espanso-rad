...
- `scripts/` — standalone Python utilities, not part of any runtime pipeline:
  ...
  - `build_release.sh` — assembles the release artifacts published by `.github/workflows/release.yml`: a base `match/`+`config/` zip, portable Espanso bundles (Windows/Linux/macOS, binaries downloaded from the latest `espanso/espanso` GitHub release), and a portable BeefText bundle (downloaded from the latest `xmichelo/Beeftext` release, pre-loaded with a generated `comboList.json`). Runs `check_matches.py` first and aborts on failure. Usage: `scripts/build_release.sh <version> [--with-portable]`.
